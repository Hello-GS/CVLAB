...
class WeatherDataset(Dataset):
    ...
    def __init__(self,label_list,transforms=None,mode="train"):
        ...
        if self.mode == "test":
            ...
            for index,row in label_list.iterrows():
                ...
                imgs.append((row["FileName"], row["label"],row['incident_ids'],row['image_name']))
            self.imgs = imgs
    def __len__(self):
    ...
    def __getitem__(self,index):
        if self.mode == "test":
            ...
            filename,label,incident_id,image_name = self.imgs[index]
            img = Image.open(filename).convert('RGB')
            img = self.transforms(img)
            return img,label,incident_id,image_name
