...
from imgs_cls.utils.logger import *
from imgs_cls.utils.losses import *
from progress.bar import Bar
from imgs_cls.utils.reader import WeatherDataset
...
def main():
    ...
    for epoch in range(start_epoch, configs.epochs):
        ...
        print('train_loss:%f, val_loss:%f, train_acc:%f, train_5:%f, val_acc:%f, f1-score:%f' % (
        train_loss, val_loss, train_acc, train_5, val_acc, float(val_f1)))
        ...
        }, is_best, is_best_loss, is_best_f1,False)
...
def train(train_loader, model, criterion, optimizer, epoch):
    ...
    for batch_idx, (inputs, targets,incident_id_list, image_name_list) in enumerate(train_loader):
...
def validate(val_loader, model, criterion, epoch):
    ...
    all_pred = []
    with torch.no_grad():
        for batch_idx, (inputs, targets, incident_id_list, image_name_list) in enumerate(val_loader):
            # measure data loading time
            data_time.update(time.time() - end)
    ...
    bar.finish()
    return (losses.avg, top1.avg,top5.avg, val_f1, val_rec, val_prec)
