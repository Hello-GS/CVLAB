...
output_path = './output_256_label0_Dhash.txt'
output_path2 = './output_256_label1_Dhash.txt'
    ...
    for file in flies:
        ...
        for graph in graphs:
            try:
                ...
                #            print(len(imagehash.dhash(Image.open(curpath),256)))
                img = Image.open(curpath)
                img = img.resize((pic_length, pic_length), Image.ANTIALIAS)
                output.write(graph + '\t' + str(imagehash.dhash(img, pic_length)) + '\n')
                count += 1
                if count % 1000 == 0:
