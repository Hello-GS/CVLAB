...
from LAB1.utils.misc import get_files
...
class WeatherTTADataset(Dataset):
    ...
    def transform_(self,data_torch,aug):
        ...
        data_torch = transforms.Normalize(mean=[0.2701, 0.0188, 0.0096],std=[0.2389, 0.0890, 0.0680])(data_torch)
        return data_torch
...
best_cpk = cpk_filename
# best_cpk = cpk_filename.replace("-checkpoint.pth.tar","-best_model.pth.tar")
# best_cpk = cpk_filename.replace("-checkpoint.pth.tar","-best_score.pth.tar")
...
with torch.no_grad():
    ...
    test_files.to_csv('./submits/%s_baseline.txt' % configs.model_name, header=None, index=None, sep=' ')
